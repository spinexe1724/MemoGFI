...
<div class="py-10 px-4">
    <div class="max-w-7xl mx-auto">
        ...
        {{-- Header --}}
        <div class="bg-gradient-to-r from-amber-600 to-amber-500 p-8 rounded-3xl text-white shadow-xl mb-8 flex justify-between items-center">
            <div>
                <h2 class="text-3xl font-black italic uppercase tracking-tight">Antrean <span class="text-amber-900">Verifikasi</span></h2>
                <p class="text-amber-50 mt-1 font-medium opacity-90 italic">Daftar pengguna baru yang menunggu penetapan Role & Divisi.</p>
            </div>
            <div class="p-4 bg-white/20 backdrop-blur-md rounded-2xl border border-white/30">
                <p class="text-[10px] font-black uppercase tracking-widest text-white mb-1">Total Menunggu</p>
                <p class="text-3xl font-black">{{ $pendingUsers->count() }}</p>
            </div>
        </div>
            ...
            <div class="p-8">
                @if($pendingUsers->count() > 0)
                    <table class="w-full text-left border-separate border-spacing-y-3">
                        <thead>
                            <tr class="text-gray-400 uppercase text-[10px] font-black tracking-[0.2em] border-b">
                                <th class="px-6 py-4">Informasi Pendaftar</th>
                                <th class="px-6 py-4">Kontak</th>
                                <th class="px-6 py-4 text-center">Cabang</th>
                                <th class="px-6 py-4 text-center">Tgl Daftar</th>
                        ...
                        <tbody class="text-gray-700">
                            ...
                            <tr class="hover:bg-amber-50/50 transition-all">
                                ...
                                <td class="px-6 py-6 bg-white border-y border-gray-50 text-center">
                                    <span class="text-sm font-bold text-gray-400">{{ $user->created_at}}</span>
                                </td>
                                <td class="px-6 py-6 bg-white border-y border-r border-gray-50 rounded-r-3xl text-right">
                                    <a href="{{ route('users.edit', $user->id) }}?from=verification" 
